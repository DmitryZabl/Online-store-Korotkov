...
class ControllerCatalogProduct extends Controller {
    ...
    if ($this->config->get('config_editor_default')) {
		...
		$data['tab_openbay'] = $this->language->get('tab_openbay');
                    ...
                    // AP TABS 
                    $data['version'] = substr(VERSION, 0, 3);

                    $data['text_ap_tabs']       = $this->language->get('text_ap_tabs');
                    $data['text_ap_add']        = $this->language->get('text_ap_add');
                    $data['text_ap_title']      = $this->language->get('text_ap_title');
                    $data['text_ap_sort']       = $this->language->get('text_ap_sort'); 
                    $data['text_tab_cross_on']  = $this->language->get('text_tab_cross_on');                     
                    $data['text_tab_cross_off'] = $this->language->get('text_tab_cross_off'); 

                    if (isset($this->error['ap_tab_name'])) {
                        $data['error_ap_tabs_name'] = $this->error['ap_tab_name'];
                    } else {
                        $data['error_ap_tabs_name'] = '';
                    }

                    if (isset($this->error['ap_tab_content'])) {
                        $data['error_ap_tab_content'] = $this->error['ap_tab_content'];
                    } else {
                        $data['error_ap_tab_content'] = '';
                    }
    ...
    $data['manufacturers'] = $this->model_catalog_manufacturer->getManufacturers();
		...
		if (isset($this->request->post['product_layout'])) {
			...
			$data['product_layout'] = array();
                    ...
                    // AP TABS 
                    if (isset($this->request->post['ap_tabs'])) { 
                        $data['ap_tabs'] = $this->request->post['ap_tabs'];
                    } elseif (isset($this->request->get['product_id'])) {
		...
		foreach ($this->request->post['product_description'] as $language_id => $value) {
			if ((utf8_strlen($value['name']) < 3) || (utf8_strlen($value['name']) > 255)) {
				$this->error['name'][$language_id] = $this->language->get('error_name');
                    ...
                    if (isset($this->request->post['ap_tabs'])) {
                        foreach ($this->request->post['ap_tabs'] as $row => $records) {
                            if (isset($records['cross'])) {
                                continue;
                            }
                            foreach ($records as $language_id => $value) {
                                if ( (utf8_strlen($value['name']) < 3) || (utf8_strlen($value['name']) > 255) ) {
                                    $this->error['ap_tab_name'][$row][$language_id] = $this->language->get('error_ap_tab_name');
                                }
                                if ( utf8_strlen($value['content']) < 3 ) {
                                    $this->error['ap_tab_content'][$row][$language_id] = $this->language->get('error_ap_tab_content');
                                }
                            }
                        }
                    }
