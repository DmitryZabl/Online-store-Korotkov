...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		$product_id = $this->db->getLastId();
                    ...
                    /* Tabs by Artem Pitov */
                    $this->db->query("DELETE FROM `". DB_PREFIX ."product_ap_tabs` WHERE product_id = '". (int) $product_id ."'");
                    $this->db->query("TRUNCATE TABLE `". DB_PREFIX ."product_ap_tabs_cross`");

                    if (isset($data['ap_tabs'])) {
                        foreach ($data['ap_tabs'] as $id => $records) {
                            if (isset($records['cross'])) {
                                unset($records['cross']);
                                foreach ($records as $language_id => $value) { 
                                    $this->db->query("
                                        INSERT INTO `". DB_PREFIX ."product_ap_tabs_cross`
                                            SET 
                                                tab_id      = '". (int) $id                                ."',
                                ...
                                foreach ($records as $language_id => $value) {
                                    $this->db->query("
                                        INSERT INTO `". DB_PREFIX ."product_ap_tabs` 
                                            SET 
                                                tab_id      = '". (int) $id                                 ."',
	...
	public function copyProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			$data['product_recurrings'] = $this->getRecurrings($product_id);
                    ...
                    // AP TABS
                    $data['ap_tabs'] = $this->getApTabs($product_id);
                    ...
                    public function getApTabs($product_id) 
                    {
                        ...
                        if ($product_tabs->num_rows) {
                            foreach ($product_tabs->rows as $tab) {
                                $tabs_data[$tab['tab_id']][$tab['language_id']] = 
                                [
                                    'name'        => $tab['name'],
                                    'content'     => $tab['content'],
                        ...
                        if ($cross_tabs->num_rows) {
                            foreach ($cross_tabs->rows as $tab) {
                                $cross_data[$tab['tab_id']]['cross'] = 1;
                                $cross_data[$tab['tab_id']][$tab['language_id']] = 
                                [
                                    'name'        => $tab['name'],
                                    'content'     => $tab['content'],
                                    'sort_order'  => $tab['sort_order']
